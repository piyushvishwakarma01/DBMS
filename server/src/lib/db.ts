import { Request, Response, NextFunction } from 'express';atabase"

export class AppError extends Error {} from "../utils/error-handler"} from "../types"
  statusCode: number;en, generateRefreshToken, verifyRefreshToken } from "../utils/jwt-utils"
  reshTokenResponse } from "../types"
  constructor(message: string, statusCode: number) {ort jwt from "jsonwebtoken"
    super(message); name: string;
    this.statusCode = statusCode;interface LoginData {ies
  }
}
}xport class NgoService {nection
export const errorMiddleware = (rId: string) {n testConnection() {
  err: AppError | Error,ase
  req: Request,ord: string): Promise<string> {
  res: Response,turn bcrypt.hash(password, 10)
  next: NextFunction,        *,le.log('✅ Prisma connection successful')
) => {        user:users (
  if (err instanceof AppError) {d: string, hashedPassword: string): Promise<boolean> {
